...
-(void)replaceImage:(NSString *)imagePath
    ...
    [self replaceImage:imagePath withUrl:urlStr withSubStr:subStr isAllReplace:NO];
}
...
-(void)replaceImage:(NSString*)imagePath
    ...
    if (range.length>0)
    {
        ...
        if (isAllReplace) {
            //替换所有 --> stringByReplacingOccurrencesOfString: withString:
            textStr = [textStrM stringByReplacingOccurrencesOfString:subStr withString:imageStr];
        } else {
            //只替换第一个 --> stringByReplacingOccurrencesOfString: withString: options: range:
...
-(void)replaceAtStr:(NSString *)atStr
    ...
    [self replaceAtStr:atStr AtStrColorR:r G:g B:b A:a Url:urlStr SubStr:subStr isAllReplace:NO];
}
...
-(void)replaceAtStr:(NSString*)atStr
    ...
    if (isAllReplace) {
        js = [js stringByReplacingOccurrencesOfString:subStr withString:atStrJs];
    } else {
